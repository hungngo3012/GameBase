...
namespace NinthArt
{
	internal class Analytics : Singleton<Analytics>
	{
		...
		private static void LogFirebaseEvent(string eventName)
		{
			if (GameManager.FirebaseOk)
			{
				try
				{
					...
					//GameManager.ShowNoti("Firebase log event: " + eventName);
				}
				catch (Exception e)
				{
					Debug.Log("Firebase analytics exception: " + e.ToString());
					//GameManager.ShowNoti("Firebase analytics exception: " + e.ToString());
				}
			}
			else
			{
				//GameManager.ShowNoti("Firebase not Ok");
		...
		internal static void LogLevelStartEvent()
		{
			...
			LogFirebaseEvent($"start_level_{Profile.Level:D3}");
#endif
		...
		internal static void LogLevelCompleteEvent()
		{
			...
			LogFirebaseEvent($"win_level_{Profile.Level:D3}");
#endif
		}
